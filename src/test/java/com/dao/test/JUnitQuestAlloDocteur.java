...
public class JUnitQuestAlloDocteur {
    ...
    // Compter le nombre d'utilisateurs dans votre source de données.
    private static final int NB_UTILISATEURS_LIST = 28;
    private static final int NB_ADRESSES_LIST = 82;
    private static final int NB_UTILISATEURS_FIND_BY_PRENOM = 3;
    private static final int NB_UTILISATEURS_FIND_BY_NOM = 4;
    private static final int NB_UTILISATEURS_FIND_BY_CODE_POSTAL = 24;
    ...
    private static final int NB_ADRESSES_FIND_BY_VILLE = 27;
    private static final int NB_ADRESSES_FIND_BY_CODE_POSTAL = 27;
    ...
    public void verifyDataUtilisateur(Utilisateur user) {
        ...
        if (user != null) {
            ...
            for (Adresse adr : user.getAdresses()) {
                ...
                if (!user.getIdUtilisateur().equals(adr.getIdUtilisateur())) {
    ...
    public void testFindByCriteria() {
        ...
        if (utilisateursByPrenom != null) {
            ...
            Assert.fail("Aucun utilisateur avec le prenom '" + UTILISATEUR_FIND_BY_PRENOM + "' n'a ete trouve dans votre source de données");
        }
        if (utilisateursByNom != null) {
            ...
            Assert.fail("Aucun utilisateur avec le nom '" + UTILISATEUR_FIND_BY_NOM + "' n'a ete trouve dans votre source de données");
        }
        if (utilisateursByCodePostal != null) {
            ...
            Assert.fail("Aucun utilisateur avec le code postal '" + UTILISATEUR_FIND_BY_CODE_POSTAL + "' n'a ete trouve dans votre source de données");
        }
        if (adressesByVille != null) {
            ...
            Assert.fail("Aucune adresse avec la ville '" + ADRESSES_FIND_BY_VILLE + "' n'a ete trouve dans votre source de données");
        }
        if (adressesByCodePostal != null) {
            ...
            Assert.fail("Aucune adresse avec le code postal '" + ADRESSES_FIND_BY_CODE_POSTAL + "' n'a ete trouve dans votre source de données");
        }
        log.debug("Sortie de la methode");
    }
    ...
    public void testCreateUpdateDeleteAdresse() {
        ...
        Assert.assertEquals(adresseCRUD.getIdUtilisateur(), userCRUD.getIdUtilisateur());
